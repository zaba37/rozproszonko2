                ...
                <div class="sidebar_container">       
                            ...
                            <c:if test="${empty sessionScope.login}">
                                ...
                                <div class="sidebar_item">
                                    <form action="search" method="post">
                                        ...
                                        <p>
                                            <input type="text" name="searchWords" id="search" value="" />
                                        <p><input type="radio" name="typ" value="and" />Ogłoszenia zawierajace wszystkie wpisane słowa</p>
                                        <p><input type="radio" name="typ" value="or"  checked="checked"/>Ogłoszenia zawierajace chociaż jedno z wpisanych słow</p>
                                        <p><input type="radio" name="typ" value="not" />Wszystkie ogłoszenia które nie zawierają wpisanych słów</p>
                                        <input type="submit" name="searchbt" value="Wyszukaj" />
                                        </p>
                                    </form>                                
                                </div><!--close sidebar_item--> 
                            </div><!--close sidebar-->     
                    ...
                    <div id="content">
                        <div class="content_item">
                            <c:if test="${empty ads}">                                             
                                <c:forEach var="add" items="${ad}">
                                    <table class="tg">
                                        <tr>
                                            <th class="tg-3iwn"> <a href="detailsAd?${add.id}"> <font color="white">${add.title}</font> </a> </th>
                                        </tr>
                                        <tr>
                                            <td class="tg-qsvf">${add.descryption}</td>
                                        </tr>
                                        <tr>
                                            <td class="tg-2ktp"><fmt:message key='kontakt'/> ${add.owner.email}</td>
                                        </tr>
                                        <tr>
                                            <td class="tg-2ktp"><fmt:message key='odwiedziny'/> ${add.visit}</td>
                                        </tr>
                                    </table>
                                    <h1></h1>
                                </c:forEach>
                                <p>
                                    <c:forEach begin="1" end="${sessionScope.howManyPages}" varStatus="loop">
                                        <a href="showAllAd?${loop.index}">${loop.index}</a>
                                    </c:forEach>
                                </p>
                            </c:if>

                            <c:if test="${not empty ads}">
                                <c:forEach var="add" items="${adsearch}">
                                    <table class="tg">
                                        <tr>
                                            <th class="tg-3iwn"> <a href="detailsAd?${add.id}"> <font color="white">${add.title}</font> </a> </th>
                                        </tr>
